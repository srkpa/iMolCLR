...
class iMolCLR(object):
    ...
    def _load_pre_trained_weights(self, model):
        try:
            checkpoints_folder = self.config["resume_from"]
    ...
    def extract_embeddings(self):
        ...
        _, _, loader = self.dataset.get_data_loaders()
        model = GINet(**self.config["model"]).to(self.device)
        model = self._load_pre_trained_weights(model)
        # print(model)
        model.eval()
        ...
        with torch.no_grad():
            for xis in loader:
                ...
                embeddings = model(xis)
                embeddings_list.append(embeddings.cpu().numpy())

        # Convertir en un seul tableau numpy
...
def main():
    ...
    res = molclr.extract_embeddings()
    np.save(f"pretrained/ckpt/DB-iMolCLR-smi.npy", res)
